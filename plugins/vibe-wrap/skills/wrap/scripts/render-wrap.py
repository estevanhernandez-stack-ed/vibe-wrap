...
def render_banner_line(breadcrumbs: list[dict]) -> str:
    """The doc's subtitle line — honest about what the wrap actually read.

    The breadcrumb-trail claim only holds when breadcrumbs actually
    contributed. With zero breadcrumbs the wrap is reconstructed from
    multi-repo git state + sibling logs — say that, rather than claiming a
    trail that wasn't there (the condition is deterministic at render time).
    """
    if breadcrumbs:
        return (
            "Generated by vibe-wrap from the breadcrumb trail your toolkit "
            "left this session."
        )
    return (
        "Reconstructed by vibe-wrap from multi-repo git state + sibling logs "
        "(no breadcrumbs found this session)."
    )
...
def assemble_doc(
    ...
    rendered = template.format(
        ...
        banner_line=render_banner_line(breadcrumbs),
